...
pdf('outputs/2022/Rates_corrected/CH4_Plots_w_corrections.pdf')
...
# pdf('outputs/2022/Rates_corrected/Plot_compare_CH4_w_corrections_2022.pdf')
...
write_xlsx(Results_CH4_w_corrections_2022, "outputs/2022/Rates_corrected/Results_CH4_w_corrections.xlsx")
...
pdf('outputs/2022/Rates_corrected/N2O_Plots_w_corrections.pdf')
...
write_xlsx(Results_N2O_w_corrections_2022, "outputs/2022/Rates_corrected/Results_N2O_w_corrections.xlsx")

##### 4.3 Loops for CO2 ####

# pdf('outputs/2022/Rates_corrected/CO2_Plots_w_corrections.pdf')

for (k in 1:length(Emission_rates_w_corrections_2022$Code)) {
  Code_k <- Emission_rates_w_corrections_2022$Code[k]
  Filt_k <- filter(Chrom_results_w_corrections_2022,Chrom_results_w_corrections_2022$Code == Code_k) # if returned as Time-Series, re-run library(dplyr)
  
  ## Loop section 1: Rate calculation.
  lm_k <- lm(CO2_byMass_mgm2~Sample_time_min, data=Filt_k)
  Emission_rates_w_corrections_2022$CO2_flux_mgm2h[k] <- coef(lm_k)[2]*60 # Returns CO2_flux_mgm2h for each Code.
  Emission_rates_w_corrections_2022$R2_CO2[k] <- summary(lm_k)$r.squared # Returns R2_CO2 for each Code.
  ...
  Emission_rates_w_corrections_2022$p_CO2[k] <- lmp_k(lm_k) # Returns p-value for each Code.
  
  # ## Loop section 2: Rate correction.
  # #### Loop section 2.1: Parameters for Outliers restriction:
  # 
  # Filt_out_k <- subset(Chrom_results_w_corrections_2022, Code == unique(Code)[k], select=c(CO2_byMass_mgm2, Sample_time_min)) 
  ...
  # Emission_rates_w_corrections_2022$CO2_flux_corrected[k] <- if_else(Emission_rates_w_corrections_2022$R2_CO2[k]<0.3, 0, ## Restriction 1: R2 < 0.3 are removed.
  #                                                 if_else(Emission_rates_w_corrections_2022$R2_CO2[k]<0.7 & (length(RemOut_k) != length(Filt_out_k$CO2_byMass_mgm2)), CO2_flux_OutRem_k, ## Restriction 2: Outliers removal
  #                                                         Emission_rates_w_corrections_2022$CO2_flux_mgm2h[k]))
  # 
  # ## Loop section 2.4: Calculating j2 according to the applied correction (if any):
  # Emission_rates_w_corrections_2022$R2_CO2_corrected[k] <- if_else(Emission_rates_w_corrections_2022$R2_CO2[k]<0.3, 0,
  #                                               if_else(Emission_rates_w_corrections_2022$R2_CO2[k]<0.7 & (length(RemOut_k) != length(Filt_out_k$CO2_byMass_mgm2)), CO2_R2_OutRem_k,
  #                                                       Emission_rates_w_corrections_2022$R2_CO2[k]))
  # 
...
write_xlsx(Emission_rates_w_corrections_2022, "outputs/2022/Rates_corrected/Emission_rates_w_corrections_2022.xlsx")

save(Emission_rates_w_corrections_2022, file = "outputs/2022/Rates_corrected/Emission_rates_w_corrections_2022.RData")
